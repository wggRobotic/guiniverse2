...
ImVec2 imgui_joystick(const char* label, float size, ImVec2 dead_ranges, ImVec2* position, ImU32 color) {
    ...
    else if (ImGui::IsItemActive()) 
    {
        ...
        joystick_position.y = 1.0f - 2.0f * ((mouse_pos.x - cursorPos.x) / size);
        joystick_position.x = 1.0f - 2.0f * ((mouse_pos.y - cursorPos.y) / size);
    ...
    ImVec2 handle_pos = ImVec2(
        cursorPos.x + (1.0f - joystick_position.y) * 0.5f * size, 
        cursorPos.y + (1.0f - joystick_position.x) * 0.5f * size
    );
